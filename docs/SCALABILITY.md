...
The Local work tray scans at most 50 displayed notes and each client's bounded draft/outbox arrays. Its straightforward filtering costs `O(N × (Q + D))` with `N ≤ 50`, `Q ≤ 100`, and `D ≤ 50`. The list uses a scrollable region; it does not grow the model or retain historical entries. At larger scale, derive the index with note-ID maps and virtualize it after measurement.
